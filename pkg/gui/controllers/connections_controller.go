...
func NewConnectionsController(
	...
	confirm := func(_ commands.ExecCtx) error {
		...
		// Hand the profile to the orchestrator's CONNECTING seam, which
		// pushes the full-pane CONNECTING screen and starts the dial on the
		// gocui MainLoop (this handler runs there). The connectGen bump that
		// anchors cancel supersession happens inside startAttempt on the
		// loop — the controller no longer dials directly (epic
		// dbsavvy-e53.5; see connectInvoker.startAttempt).
		if base.helpers.OnBeginConnecting != nil {
			base.helpers.OnBeginConnecting(profile)
		}
		return nil
	}
